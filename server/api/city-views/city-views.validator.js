...
const validateUpdateCityViewReqBody = async (req, res, next) => {
  const cityView = {
...
const validateUpdateCityViewImage = async (req, res, next) => {
  if (typeof req.files !== 'undefined' && req.files !== null && req.files.imageFile) {
    if (VALID_MIMETYPE.includes(req.files.imageFile.mimetype) === false) {
      res.status(400).send(({ code: 400, status: 'BAD_REQUEST', message: `File should be one of [ ${VALID_MIMETYPE} ] mimetype` }));
      return;
    }
...
module.exports = {
  ...
  validateUpdateCityView: validateUpdateCityViewReqBody,
  validateId: validateCityViewIdInURL,
  validateUpdateCityViewImage,
