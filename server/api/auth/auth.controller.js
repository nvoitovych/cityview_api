...
const registerUser = async (req, res) => {
  ...
  const hashedPassword = await bcrypt.hash(password, 10).catch((error) => {
    console.error('registerUser | hashedPassword | error: ', error);
    switch (error.code) {
      default: {
  ...
  const resultUser = await user.create({
    ...
    console.error('registerUser | resultUser | error: ', error);
  ...
  const generatedToken = await generateEmailConfirmationToken(resultUser).catch((error) => {
    console.error('registerUser | generatedToken | error: ', error);
